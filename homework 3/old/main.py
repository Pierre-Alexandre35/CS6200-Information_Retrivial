...
from pqueue import Node


seedUrls = ["http://www.nhc.noaa.gov/outreach/history/", 
...
for seedurl in seedUrls:
    urlNode = Node(seedurl, score);
    queue.insert(urlNode)
    print(queue.show())
queue.delete()
print(queue.show())
...
#             response = requests.get(url)
#             if 'html' in response.headers['content-type'] and response.status_code == 200:
#                 with open(f"index{id_}.html", 'w') as html:
#                     soup = BeautifulSoup(response.content, 'html.parser')
#                     title_data = soup.title.string
#                     print(title_data)
#                     html.write(str(soup))
